  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (view === 'LOGIN') {
      ...
      try {
          ...
          // 3. Get User Profile (Cloud First - Prefer UID fetch to avoid Permission Denied on Queries)
          // Since we just logged in, we have the UID. Fetching the doc directly works even if List is blocked.
          let appUser: any = await getUserData(firebaseUser.uid);
          
          if (!appUser) {
              // Fallback: Try Email Query (only if UID fetch failed, though unlikely for valid user)
      ...
      try {
        ...
        if (!showAdminVerify) {
            ...
            if (adminAuthCode === settings?.adminCode) {
                try {
                    await setPersistence(auth, browserLocalPersistence);
                    // Try Anonymous Auth to satisfy security rules
                    ...
                    logActivity("ADMIN_LOGIN", "Admin Access Granted");
                    onLogin(adminUser);
                } catch (e: any) {
                    console.error(e);
